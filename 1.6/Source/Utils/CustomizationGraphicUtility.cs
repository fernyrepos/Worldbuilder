...
namespace Worldbuilder
{
    ...
    {
        // Cache for custom image textures used in previews/icons
        ...
        /// <summary>
        /// Gets the appropriate base Graphic instance based on CustomizationData priorities
        /// (custom image > variation > style > default).
        /// Intentionally does NOT handle color application; that's done during drawing.
        /// </summary>
        public static Graphic GetGraphic(ThingDef def, CustomizationData data)
        {
            if (data == null || def?.graphicData == null)
            {
                return def?.graphic; // Return default graphic if no data or graphicData
            }
            ...
            if (!string.IsNullOrEmpty(data.selectedImagePath))
            {
                ...
                // Match original logic: Check for "CustomImages/" prefix when a preset is loaded
                if (imagePath.StartsWith("CustomImages/") && WorldPresetManager.CurrentlyLoadedPreset != null)
                {
                    ...
                    // Combine preset folder with the path *after* "CustomImages/"
                ...
                // Otherwise, use imagePath as is (could be absolute or relative to game data)
                ...
                if (customTextureCache.TryGetValue(imagePath, out Texture2D cachedTex))
                {
                    ...
                    return null; // Cached as missing
                }

                // Not cached, try loading
                ...
                else
                {
                    ...
                    customTextureCache[imagePath] = MissingTexture; // Cache file not found
                    return null;
                }
            }
            ...
            // 4. Fallback to Default Graphic
            return def.graphic;
        }


        /// <summary>
        /// Draws the appropriate graphic for a ThingDef based on CustomizationData.
        /// Handles custom images, variations, styles, and applies color.
        /// </summary>
        public static void DrawCustomizedGraphicFor(Rect rect, ThingDef def, CustomizationData data, float iconAngle = 0f, float iconDrawScale = 1f, Color? overrideColor = null)
        {
            ...
            Color color = overrideColor ?? data?.color ?? Color.white; // Use override, then data color, then white

            // Special handling for linked graphics or doors which might rely on DefIcon logic
            bool useDefIcon = (graphic is Graphic_Linked) || (def.thingClass == typeof(Building_Door) && data?.styleDef != null);

            if (useDefIcon)
            {
                // DefIcon handles styles internally, but we need to pass the correct one if applicable
                ...
                // Use positional arguments: DefIcon(Rect, BuildableDef, ThingDef, scale, ThingStyleDef, drawPlaceholder, color)
                Widgets.DefIcon(rect, def, null, iconDrawScale * 0.85f, styleToUse, false, color); // Corrected arguments
            }
            else if (graphic != null)
            {
                Material material = graphic.MatAt(def.defaultPlacingRot); // Use default rotation
                if (material != null)
                {
                    ...
                    if (resolvedTexture != null)
                    {
                        ...
                        Widgets.ThingIconWorker(
                            ...
                            resolvedTexture,
                            iconAngle,
                            iconDrawScale * 0.85f // Match scale used elsewhere
                        );
                        GUI.color = Color.white;
                    }
                    else { Widgets.DrawBox(rect); } // Draw box if texture missing
                }
                else { Widgets.DrawBox(rect); } // Draw box if material missing
            }
            else
            {
                // Fallback if GetGraphic returned null (e.g., failed image load)
                // Use positional arguments: DefIcon(Rect, BuildableDef, ThingDef, scale, ThingStyleDef, drawPlaceholder, color)
                Widgets.DefIcon(rect, def, null, iconDrawScale * 0.85f, null, false, color); // Corrected arguments
            }
        }
    }
}
